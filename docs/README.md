...
# 或构建 Docker 镜像
docker build -t dujiao-rust:latest .
...
1. **Cloudflare Tunnel**（`cloudflared`）—— 推荐：无需开放公网端口，无防火墙缺口。
   ```bash
   cloudflared tunnel create dujiao
   cloudflared tunnel route dns dujiao your-domain.example.com
...
1. 访问 `http://your-host/install` —— 当 `bootstrap_password` 仍是默认值 `admin123456` 时，程序**拒绝**自动初始化管理员。使用 `/install` 创建一个强密码的 owner。若想强制保留旧的自动初始化行为（不推荐），设置 `DUJIAO_ALLOW_DEFAULT_ADMIN=1`。
2. 进入 `/admin/settings`：
...
| `data/dujiao.db` | SQLite 数据库（WAL 模式 + 外键开启）。可在 `/admin/backup` 安全备份（一致性 SQLite 快照 gzip 流） |
...
| 变量 | 用途 |
| --- | --- |
| `DUJIAO_CONFIG` | TOML 配置路径（默认 `./config.toml`） |
| `DUJIAO_ENABLE_INSTALL` | 即使设置了强密码，也强制跳过自动初始化 |
| `DUJIAO_ALLOW_DEFAULT_ADMIN` | 默认密码下重新启用自动初始化（不推荐） |
| `RUST_LOG` | tracing 过滤器，例如 `dujiao_rust=debug,tower_http=info` |
...
数据库中的敏感字段通过 `data/app.secret` 中自动生成的本地密钥加密。迁移或恢复时，请将 `data/app.secret`、`data/dujiao.db`、`uploads/` 一并保留；丢失密钥可能导致加密的 SMTP、通知设置需要重新填写。
...
本节说明 `dujiao-rust` 后台支付通道的技术框架和接入配置方式。面向部署和运营配置，不展开代码实现细节。

## 1. 支付通道框架

`dujiao-rust` 使用统一的支付通道模型管理不同支付平台。每个通道由三部分组成：
...
| 字段 | 说明 |
| --- | --- |
| 支付类型 | 先选择 `模拟`、`内置` 或 `外部`。模拟只用于本地联调；内置是由 dujiao-rust 自己处理的收款能力；外部是第三方支付网关。 |
| 支付方式 | 在所选支付类型下选择具体方式。当前内置方式为 `基于 Alchemy ERC20 Token`，对应 Provider `evm-local`。 |
| 显示名称 | 前台展示的支付方式名称，例如"支付宝"、"USDT-TRC20"。 |
| 支付提供方 | Provider 类型。当前常用值：`noop`、`epay`、`tokenpay`、`evm-local`、`epusdt`、`bepusdt`、`dujiaopay`、`okpay`、`official`。 |
| 渠道类型 | 具体支付渠道。常见值：`alipay`、`wxpay`、`qqpay`、`usdt`、`trx`、`stripe`、`paypal`、`wechat`、`test`。 |
| 旧支付标识 `pay_check` | 兼容独角数卡前台图标和旧支付标识，建议填写 `alipay`、`wxpay`、`qqpay`、`usdt` 等。 |
...
DujiaoPay 是 dujiao-rust 推荐的多链虚拟币收款 SaaS。Provider 固定为 `dujiaopay`，渠道类型必须使用平台定义的 `token_id`。
...
支持的 `token_id` → 链映射（来自 `resolve_dujiaopay_chain`）：
...
完整配置字段（来自 `DujiaoPayProvider`）：

| 字段 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `api_base_url` | ✓ | `https://www.dujiaopay.com` | DujiaoPay API 根地址；自托管时改成自己的入口 |
...
| 类型 | 地址 |
| --- | --- |
| 异步通知（推荐） | `/payment/callback/dujiaopay/{token_id}` |
| 异步通知（兼容旧路由） | `/pay/dujiaopay/notify_url` |
...
| `dujiaopay` | `token_id` | 沙箱或小额实付 | 待验收 | 验证后启用 |
