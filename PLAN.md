...
```text
dujiao-rust/
  dujiao-rust              # 编译后的 binary
  config.toml              # 可选运行配置
  data/
    dujiao.db              # SQLite 数据库
  uploads/
    products/
    admin/
  logs/
...
| P0 | 搜索密码和浏览器 Cookie 查单不完整 | `is_open_search_pwd`、`dujiaoka_orders` Cookie | 下单写 Cookie，邮箱查单校验密码，浏览器 Cookie 查单 |
...
- 下单后把订单号追加到 `dujiaoka_orders` Cookie。
- 邮箱查单最多返回最近 5 条。
...
1. `create_order` 成功后设置 `dujiao_orders` 和兼容名 `dujiaoka_orders`。
...
- `dujiao-rust import dujiaoka --mysql-url ... --dry-run`
- `dujiao-rust import dujiaoka --sql-file install.sql --dry-run`
- `dujiao-rust import dujiaoka --mysql-url ... --apply`
...
- `PricingService`：
  ...
  - 结算页展示原价、批发优惠、优惠码优惠。
- 查单：
  - 下单后写入兼容 Cookie `dujiaoka_orders`。
  - 支持订单号查单。
  - 支持邮箱 + 查询密码查单。
  - 支持浏览器 Cookie 查单。
- 人工商品：
...
- 安装初始化页：
  ...
  - 新增 `POST /do-install`。
  - 已初始化时只提示进入后台登录。
  - 若希望首次启动不自动创建默认管理员，可设置 `DUJIAO_ENABLE_INSTALL=1` 后启动，再通过安装页创建 owner 管理员。
- 通知渠道：
...
本轮目标：将 `/data/projects/dujiao-next` 中已注册的 10 个外部支付 adapter 移植到 `dujiao-rust`，继续保持单 binary、SQLite、无 Redis。
...
- 通道实现：
  ...
  - DujiaoPay：HMAC API 下单、DJP webhook 验签。
...
- 后台：
  - 支付通道下拉补齐 `bepusdt`、`dujiaopay`、`okpay`、`official`。
  - 旧字段 `merchant_id/merchant_key/merchant_pem` 自动映射到各 provider 常用配置键。
- 文档：
  - `/docs/payment-channels.md` 补充 Bepusdt、DujiaoPay、Okpay、Stripe、PayPal、官方支付宝、官方微信支付配置说明。
...
- Luna 主题：
  ...
  - `assets/luna/main.js` 增加 `stripe`、`tokenpay`、`epusdt`、`bepusdt`、`dujiaopay`、`okpay`、`usdt`、`usdc`、`trx`、`wechat` 等映射。
...
| 步骤 | 结果 |
| --- | --- |
| 创建订单 | 303 → `/bill/<order_no>`，set-cookie `dujiaoka_orders` |
...
- 响应头 `Content-Type: application/gzip` + 文件名 `dujiao-backup-<timestamp>.sqlite.gz`
...
- 旁路通道 `DUJIAO_ALLOW_DEFAULT_ADMIN=1` 仅 dev/CI 使用
...
- 新 `security/csrf.rs`：
  - 全局中间件，在 GET 响应自动 `Set-Cookie: dujiao_csrf=…; Path=/; SameSite=Lax[; Secure]`
  - POST/PUT/PATCH/DELETE 严格校验 `Cookie == _csrf 查询/x-csrf-token 头`
  - 常数时间比较，防 timing 攻击
- Cookie 不再硬编码 `Path=/admin`，全站 Cookie 让前端表单与后台表单都能携带
- 已验证：`set-cookie: dujiao_csrf=…; Path=/; SameSite=Lax; Max-Age=43200; Secure`；POST 缺 cookie 返回 403

**B5b：敏感字段加密落库**

- 新 `security/secrets.rs::SecretBox`：
  - SHA-256(`"dujiao-rust/v1/" || app_secret`) → 256-bit AES-GCM key
  - 输出格式 `enc:v1:<base64(12B nonce || ciphertext)>`
  - 空字符串透传；旧明文记录读取时也透传（迁移兼容）
  - 4 个 `#[cfg(test)]` 单元测试
- `config::AppConfig::effective_app_secret()`：优先级 `admin.app_secret` > `DUJIAO_APP_SECRET` env > 派生 fallback
...
- 新增 `src/lib.rs` 暴露模块（`main.rs` 改用 `dujiao_rust::*`）
...
- 强制 `DUJIAO_APP_SECRET` env 必填，无默认值
...
**P1-1 配套：systemd unit（`deploy/dujiao-rust.service`）**

- `User=dujiao`、`KillMode=mixed`、`KillSignal=SIGTERM`、`TimeoutStopSec=20s` 配合 P1-4 graceful
- 安全 hardening：`NoNewPrivileges`、`ProtectSystem=strict`、`ProtectHome`、`ReadWritePaths` 精确白名单
- `EnvironmentFile=/etc/dujiao-rust/secrets.env` 0600 保护 `DUJIAO_APP_SECRET`
...
| `GET /buy/1` 响应头含 `set-cookie: dujiao_csrf=...; Secure` | ✓ |
...
- 生产配置仍依赖人工正确设置：`DUJIAO_APP_SECRET`、`site.base_url`、`cookie_secure`、`trust_proxy_hops`、后台默认密码/安装状态。
...
- `/admin/backup` 当前实现直接读取 `data/dujiao.db` 并 gzip。
- WAL 模式下最新提交数据可能仍在 `dujiao.db-wal`，直接复制主库文件不是可靠热备份。
...
- Dockerfile 当前 `HEALTHCHECK` 调用 `/app/dujiao-rust --healthcheck`，但二进制未实现该参数。
...
- Dockerfile 保留 `HEALTHCHECK CMD ["/app/dujiao-rust", "--healthcheck"]`。
- docker-compose healthcheck 改为同样调用 `/app/dujiao-rust --healthcheck`，去掉对 `wget` 的依赖。
- `/healthz` 保留给外部负载均衡和人工检查。

验收：

```bash
/app/dujiao-rust --healthcheck
...
| dujiaopay | token_id | 待沙箱/小额 | 待验证 webhook | 待验证 | 待验证 | 验证后启用 |
...
- 检查项：
  - `DUJIAO_APP_SECRET` 或 `admin.app_secret` 是否显式设置，长度是否足够。
...
- R3 容器健康检查：
  ...
  - Dockerfile 保留并使用 `/app/dujiao-rust --healthcheck`。
  - docker-compose healthcheck 改为 `/app/dujiao-rust --healthcheck`，不再依赖 slim 镜像内不存在的 `wget`。
- R4 支付通道验收矩阵：
  - 新增 `docs/payment-provider-acceptance.md`。
...
./target/debug/dujiao-rust --healthcheck # exit 0
sqlite3 data/dujiao.db "VACUUM INTO 'data/backup-tmp/manual-check.sqlite';"
...
- 敏感配置加密密钥改为自动生成、持久化并定期换新，不再要求用户配置 `DUJIAO_APP_SECRET`。
...
- 用户不需要设置 `DUJIAO_APP_SECRET`。
- 首次启动如果 `data/app.secret` 不存在，由程序自动生成 32 字节随机密钥并写入 0600 文件权限。
- 密钥需要随 `data/dujiao.db` 和 `uploads/` 一起迁移、备份和恢复。
...
- 下载历史备份时校验文件名必须以 `dujiao-backup-` 开头、以 `.sqlite.gz` 结尾，且不允许 `/`、`\\`、`..`。
...
本节响应“`DUJIAO_APP_SECRET` 能否删除，不通过用户设置，改成自动生成，并定时换新”的要求。

设计结论：

- 可以删除用户必须配置 `DUJIAO_APP_SECRET` 的要求。
- 不能每次启动随机生成临时密钥；否则已经加密落库的 SMTP、通知配置会无法解密。
- 最优实现是本地持久密钥文件：首次启动自动生成 `data/app.secret`，后续启动复用。
- 旧配置中的 `admin.app_secret` / `DUJIAO_APP_SECRET` 仅作为兼容迁移来源，用于首次创建 `data/app.secret` 时把旧密钥加密的数据重加密到新密钥。
...
- 后台生产就绪检查：
  ...
  - 不再把缺少 `DUJIAO_APP_SECRET` / `admin.app_secret` 当作 blocker。
- 文档与部署：
  - README、Docker Compose、systemd 示例不再要求配置 `DUJIAO_APP_SECRET`。
  - 备份/恢复说明改为必须保留 `data/app.secret`。

验收标准：

- 不设置 `DUJIAO_APP_SECRET` 也能启动并创建 `data/app.secret`。
...
- `src/security/jwt.rs`：
  - `Jwt::from_app_secret`：SHA-256 派生 256-bit HS256 key（与 `SecretBox` 同根，但用独立 domain separator `"dujiao-rust/jwt/v1/"`）
...
- `package.json` name 改 `dujiao-rust-admin`，version `0.1.0`
- `.env`：`VITE_BASE_URL=/admin/`、`VITE_APP_TITLE="Dujiao Rust 后台"`、`VITE_AUTH_ROUTE_MODE=static`、`VITE_STATIC_SUPER_ROLE=owner`、`VITE_SERVICE_SUCCESS_CODE=0000`、`VITE_SERVICE_LOGOUT_CODES=4002,8888,8889`、`VITE_SERVICE_EXPIRED_TOKEN_CODES=9999`、`VITE_STORAGE_PREFIX=DUJIAO_`、`VITE_FORCE_DEFAULT_LANG=zh-CN`
...
| 请求 | 结果 |
| --- | --- |
| `GET /admin` / `/admin/` | 200，HTML title `<title>Dujiao Rust 后台</title>` |
...
| `GET /admin/` SPA index | 200，title `Dujiao Rust 后台` |
...
| `GET /admin/setup` SPA | 200，title `Dujiao Rust 后台`，vue-router 加载 setup view |
...
3. 用户填表（默认 `admin/Administrator/独角数卡/Dujiao Rust`）→ 提交
...
`bash build.sh && setsid nohup ./target/debug/dujiao-rust &`
...
审计结论：这 5 个 provider 在 dujiao-rust 从未有过支付实现代码（`src/payment/providers.rs` 与 registry 均无注册），残留仅为 Coinbase 的 `pay_check` 图标标识痕迹。已完成的清理（工作树已改，尚未 `build.sh` 重编译进 binary）：
...
当前 dujiao-rust 的实际防刷手段是：算术图形验证码（`captcha_challenges` 表 + SVG）、邮箱/IP 限购频控（`purchase_rate`）、登录失败锁定。这三层是真实生效的。
