...
<p align="center"><strong>A single-binary Rust order/card-secret storefront rebuilt from Dujiaoka, without Redis or MySQL.</strong></p>
...
`dujiao-rust` reimplements the core card-selling flow of [Dujiaoka](https://github.com/assimon/dujiaoka) in Rust. The public storefront keeps the original `luna` / `hyper` / `unicorn` server-rendered themes, while the admin UI is an embedded Vue SPA. Payments, card secrets, fulfillment, and settings all live in one process and one SQLite database.
...
- Single-binary deployment: `dujiao-rust` serves the SSR storefront, payment callbacks, embedded worker, and admin APIs. The admin SPA is built and embedded into the binary via `rust-embed`.
- Storefront parity with Dujiaoka: three themes (`luna` / `hyper` / `unicorn`), familiar routes, and the original purchase flow rendered with `minijinja`.
...
- Payment provider registry: unified `payments` table and idempotent callback validation. Built-in channels include `epay`, `tokenpay`, `epusdt`, `bepusdt`, `dujiaopay`, `okpay`, `evm-local`, and official Alipay/WeChat/Stripe/PayPal integrations.
...
Preserve `data/app.secret`, `data/dujiao.db`, and `uploads/` together when migrating or restoring. Losing the secret key means encrypted SMTP/notification settings may need to be re-entered.
...
| Environment | `DUJIAO_CONFIG`, `DUJIAO_ALLOW_DEFAULT_ADMIN`, `RUST_LOG` |
...
- [systemd unit example](deploy/dujiao-rust.service)
