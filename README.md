...
<p align="center"><strong>基于 Rust 的单二进制订单/卡密发卡系统，单进程 SQLite 部署，无需 Redis 或 MySQL。</strong></p>
...
`freeMarket`（二进制名 `free-market`）是一套 Rust 实现的订单/卡密发卡系统：前台沿用 `luna` / `hyper` / `unicorn` 三套主题的服务端渲染页面，后台为内嵌 Vue SPA，支付、卡密、履约与设置全部落在同一个进程和 SQLite 数据库中。
...
- 单二进制部署：`free-market` 同时承载前台 SSR、支付回调、内嵌 worker 与管理后台 API；admin SPA 构建后通过 `rust-embed` 编入 binary。
- 成熟前台主题：`luna` / `hyper` / `unicorn` 三套主题、完整购买流程；商品展示、下单、账单、支付、查单页面由 `minijinja` 渲染。
- 卡密状态机：`available` → `reserved` → `used`，下单先预占、超时释放、支付后履约，降低超卖风险。
- 支付 Provider 注册表：统一 `payments` 表与幂等回调校验，内置 `epay`、`tokenpay`、`epusdt`、`bepusdt`、`freemarketpay`、`okpay`、`evm-local`、官方支付宝/微信/Stripe/PayPal 等通道。
...
- 设置持久化：`settings` 表保存站点、订单、主题、验证码、安全、SMTP、通知等 JSON 配置；敏感字段用 `$HOME/.free-market/app.secret` 本地加密。
...
部署方式：编译 release 二进制后通过 systemd 运行（见 `deploy/free-market.service`），前置 Cloudflare 处理 TLS。
...
迁移或恢复时请一并保留 `$HOME/.free-market/` 下的 `app.secret`、`free-market.db`、`uploads/`；丢失密钥会导致加密的 SMTP/通知配置需重新填写。

## 关键配置

静态配置写在 `config.toml`（默认可放在项目目录或 `$HOME/.free-market/config.toml`）。运行时站点/订单/支付等业务配置保存在 SQLite `settings` 表，可通过 `/admin/settings` 修改。未指定 `database.path` 时，所有数据默认落在 `$HOME/.free-market/`（含 `free-market.db`、`uploads/`、`backups/`、`app.secret`）。

| 领域 | 配置 |
| --- | --- |
| 监听 | `[server] host`、`port` |
| 数据库 | `[database] path`（默认旁路生成 `uploads/` 与 `app.secret`）；表结构版本见 `PRAGMA user_version` / `schema/` |
...
| 环境变量 | `FREEMARKET_CONFIG`、`FREEMARKET_ALLOW_DEFAULT_ADMIN`、`RUST_LOG` |
...
- [systemd 单元示例](deploy/free-market.service)
- [配置样例](config.example.toml)

## 致谢

前台主题与交互体验源自开源项目 [独角数卡 (dujiaoka)](https://github.com/assimon/dujiaoka)。freeMarket 在 Rust 侧重新实现了交易、支付与后台能力，并非 dujiaoka 的官方 fork。
